...
function star261(){
     if [ ! -d "/star261/" ]; then
        ...
        git clone -b main https://gh.api.99988866.xyz/https://github.com/star261/jd.git /star261
    else
    ...
    cp -f /star261/scripts/jd_productZ4Brand.js /scripts
     echo "2 14,20 * * * node /scripts/jd_productZ4Brand.js >> /scripts/logs/jd_productZ4Brand.log 2>&1" >> /scripts/docker/merged_list_file.sh
}
function main(){
    # 首次运行时拷贝docker目录下文件
