...
function star261(){
    ...
    cp -f /star261/scripts/jd_xinxiangyin.js /scripts
     echo "5 9 * * * node /scripts/jd_productZ4Brand.js >> /scripts/logs/jd_productZ4Brand.log 2>&1" >> /scripts/docker/merged_list_file.sh
     echo "8 12,18 1-30 9,10 * node /scripts/jd_xinxiangyin.js >> /scripts/logs/jd_xinxiangyin.log 2>&1" >> /scripts/docker/merged_list_file.sh
}
function main(){
    # 首次运行时拷贝docker目录下文件
